...
def LaserScanProcess(data):
    ...
    min_angle, max_angle = largest_gap[0] * ((data.angle_increment) *
                                             180 / PI), largest_gap[-1] * ((data.angle_increment) * 180 / PI)
...
def main():
    ...
    pub = rospy.Publisher('/thorvald_001/turtle1/cmd_vel',
                          Twist, queue_size=10)

    rospy.Subscriber("/thorvald_001/scan", LaserScan, LaserScanProcess)
